...
function dynamicSlice(start, end) {
    return function (arr){
      if (start < 0) return arr.filter(el => arr.indexOf(el) < end)
      if (end > arr.length) return arr.filter(el => arr.indexOf(el) + 1 > start)
      return arr.filter(el => arr.indexOf(el) + 1> start && arr.indexOf(el) < end)
    }
}
